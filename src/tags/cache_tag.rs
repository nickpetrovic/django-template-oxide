...
impl Node for CacheNode {
    fn render(&self, py: Python<'_>, context: &mut Context) -> Result<String, TemplateError> {
        ...
        let timeout_result = crate::nodes::resolve_expression_rust(
            py, &self.expire_time_expr, context,
        );
        let timeout_val = match timeout_result {
            ...
            Err(TemplateError::VariableDoesNotExist { .. }) | Err(TemplateError::Internal(_)) => {
                return Err(TemplateError::TemplateSyntaxError(
                    format!("\"cache\" tag got an unknown variable: {:?}", self.expire_time_expr),
                ));
            }
            Err(e) => return Err(e),
        };
        ...
        let timeout: Option<i64> = match &timeout_val {
            ...
            Value::String(s) => match s.parse::<i64>() {
                ...
                Err(_) => {
                    return Err(TemplateError::TemplateSyntaxError(
                        format!("\"cache\" tag got a non-integer timeout value: {:?}", s),
                    ));
                }
            },
            Value::SafeString(s) => match s.parse::<i64>() {
                ...
                Err(_) => {
                    return Err(TemplateError::TemplateSyntaxError(
                        format!("\"cache\" tag got a non-integer timeout value: {:?}", s.as_ref()),
                    ));
                }
            },
            _ => {
                return Err(TemplateError::TemplateSyntaxError(
                    format!("\"cache\" tag got a non-integer timeout value: {:?}", timeout_val.to_string()),
                ));
            }
        };
            ...
            let cache = if let Some(ref alias) = cache_alias {
                caches.get_item(alias.as_str()).map_err(|_| {
                    TemplateError::TemplateSyntaxError(
                        format!("Invalid cache name specified for cache tag: {:?}", alias),
                    )
                })?
            } else {
...
pub fn compile_cache(parser: &mut Parser, token: &Token) -> Result<Box<dyn Node>, TemplateError> {
    ...
    if bits.len() < 3 {
        return Err(TemplateError::TemplateSyntaxError(
            format!("'{:?}' tag requires at least 2 arguments.", bits.first().unwrap_or(&String::new())),
        ));
    }
